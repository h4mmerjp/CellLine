...
export default function App() {
  ...
  const { labelDrag, onHandleDown, onHandleUp } = useLabelDrag(dispatch, hLines, vLines, colWidths, rowHeights);
  const { selection, setSelection, selStart, onCellDown, onCellTouchStart, onCellEnter } = useCellSelection(editingRef);
  ...
  const doImport = (e) => {
    ...
    reader.onload = (ev) => {
      try { dispatch({ type: "LOAD", data: JSON.parse(ev.target.result) }); }
      catch { setErrorMsg("ファイルの読み込みに失敗しました"); setTimeout(() => setErrorMsg(null), 3000); }
    };
  ...
  const toolBtn = (color, on = true) => ({
    ...
    border: `1.5px solid ${on ? color : "#ddd"}`, borderRadius: 5,
    background: "#fff", color: on ? color : "#ccc",
    cursor: on ? "pointer" : "default", transition: "all 0.15s",
  });

  return (
    <div
      style={{ minHeight: "100vh", background: "#fafafa", fontFamily: "'Hiragino Kaku Gothic ProN','Yu Gothic',sans-serif", padding: "28px 24px", color: "#333", userSelect: resizing || labelDrag ? "none" : "auto", cursor: resizing?.type === "col" ? "col-resize" : resizing?.type === "row" ? "row-resize" : "default" }}
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 24, flexWrap: "wrap", alignItems: "center" }}>
        ...
        {selection && (
          <span style={{ fontSize: 11, color: "#999" }}>
            {selIsMulti ? `${selection.r2 - selection.r1 + 1}行 × ${selection.c2 - selection.c1 + 1}列 選択中` : canUnmerge ? `結合セル (${singleMerge.rowSpan}×${singleMerge.colSpan})` : null}
          </span>
        )}
